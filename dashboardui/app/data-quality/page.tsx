        ...
        <div className="space-y-10">
           <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
              ...
              <div className="bg-card p-8 rounded-[32px] border border-border shadow-sm transition-colors duration-300">
                 ...
                 <p className="text-sm font-bold text-foreground truncate">{data.last_loaded_at.split('.')[0]}</p>
              </div>
           </div>
                    ...
                    <tbody className="text-sm font-semibold divide-y divide-border">
                      {data.columns?.map((c: any, i: number) => (
                        <tr key={i} className="hover:bg-orange-50/10 dark:hover:bg-orange-600/5 transition-colors">
                           ...
                           <td className="py-6 px-4 text-center text-muted-foreground">{c.null_count.toLocaleString()}</td>
                           <td className="py-6 px-4 text-center text-muted-foreground">{c.null_pct}%</td>
                           <td className="py-6 px-4 text-right">
                              <span className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest ${
                                 c.status === 'OK' ? 'bg-orange-50 dark:bg-orange-600/10 text-orange-600' : 'bg-red-50 dark:bg-red-950/20 text-red-600'
                              }`}>
                                 {c.status}
                              </span>
                           </td>
